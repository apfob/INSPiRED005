...
#Hyperparamters
#degree: the maximum degree of interactions (Rarely is there any benefit in assessing greater than 3-rd degree interactions)
#nprune: the number of terms retained in the final model

hyper_grid_mars <- expand.grid(
  degree = seq(from=1, to= 3, by=1), #no decimals 
  nprune = seq(from=1, to= 10, by=1) #1
)
...
#degree nprune Accuracy     Kappa     ROC      Sens      Spec       AUC Precision    Recall         F
#      1     10 0.870994 0.6594084 0.92606 0.7243333 0.9237123 0.7908069 0.7793607 0.7243333 0.7449025
#AccuracySD   KappaSD     ROCSD    SensSD     SpecSD      AUCSD PrecisionSD  RecallSD        FSD
# 0.03646136 0.1017769 0.0243833 0.1158424 0.03351396 0.05460745  0.08287511 0.1158424 0.07999031
ggplot(cv_mars)
...
#mars 
cv_mars[["resample"]][["ROC"]] #AUROC
CI(cv_mars[["resample"]][["ROC"]], ci=0.95)
range(cv_mars[["resample"]][["ROC"]])

cv_mars[["resample"]][["Kappa"]]
CI(cv_mars[["resample"]][["Kappa"]], ci=0.95)

cv_mars[["resample"]][["Accuracy"]]
CI(cv_mars[["resample"]][["Accuracy"]], ci=0.95)

cv_mars[["resample"]][["Sens"]]
CI(cv_mars[["resample"]][["Sens"]], ci=0.95)

cv_mars[["resample"]][["Spec"]]
CI(cv_mars[["resample"]][["Spec"]], ci=0.95)

cv_mars[["resample"]][["Precision"]] #positive-predictive value
CI(cv_mars[["resample"]][["Precision"]], ci=0.95)
...
##mars 
confusionMatrix(as.factor(cv_mars[["pred"]][["pred"]]), factor(cv_mars[["pred"]][["obs"]]), positive="Malignant") #confusion matrix

#set cutoff
mars_cutoff <- as.data.frame(
  cbind(cv_mars[["pred"]][["Malignant"]], cv_mars[["pred"]][["Benign"]], factor(cv_mars[["pred"]][["obs"]]))
)
colnames(mars_cutoff) <- c("prediction_malignant","prediction_benign", "actual")
require(Rfast)
mars_cutoff <- subset(mars_cutoff, mars_cutoff$actual ==1) 
mars_cutoff = (Rfast::nth(mars_cutoff$prediction_malignant, 2, descending = FALSE))
detach(package:Rfast)

mars_binary_prediction_training <-
  ifelse(cv_mars[["pred"]][["Malignant"]] 
         >= mars_cutoff ,
         "Malignant",
         "Benign")

confusionMatrix(as.factor(mars_binary_prediction_training), factor(cv_mars[["pred"]][["obs"]]), positive="Malignant") #confusion matrix
...
roc_mars_validation = roc(as.vector(db_validation$outcome),as.matrix(predict(cv_mars , db_validation, type = "prob")$"Malignant")) #Conduct the ROC analyses
auc_mars_validation = pROC::auc(roc_mars_validation) #Calculate the area under the ROC curve
auc_CI_mars_validation = pROC::ci.auc(roc_mars_validation, method="bootstrap", boot.stratified=TRUE) #Calculate the area under the ROC curve
...
#mars
confusionMatrix(as.factor(predict(cv_mars , db_validation)), factor(db_validation$outcome), positive="Malignant") #confusion matrix

mars_binary_prediction_validation <-
  ifelse(predict(cv_mars , db_validation, type="prob")$"Malignant" 
         >= mars_cutoff,
         "Malignant",
         "Benign")

confusionMatrix(as.factor(mars_binary_prediction_validation), factor(db_validation$outcome), positive="Malignant") #confusion matrix
